		...
		if(IS_POST){
			...
			if($data){
                ...
                if($id){
                    ...
                    $this->error( '140050:新增失败');
                }
            } else {
                $this->error( '140051:' . $Model->getError());
            }
        } else {
            ...
            if($data){
                if($Model->save()!== false){
                   $this->success('更新成功', U('lists'));
                    ...
                    $this->error( '140052:更新失败');
                }
            } else {
                $this->error( '140053:' . $Menu->getError());
            }
        } else {
            $info = array();
            /* 获取数据 */
        ...
        if(IS_POST){
			...
			if($data){
                ...
                if($id){
                    ...
                    $this->error( '140057:新增失败');
                }
            } else {
                $this->error( '140058:' . $Model->getError());
            }
        } else {
            ...
            if($data){
                if($Model->save()!== false){
                   $this->success('更新成功', U('groupList'));
                    ...
                    $this->error( '140059:更新失败');
                }
            } else {
                $this->error( '140060:' . $Menu->getError());
            }
        } else {
            $info = array();
            /* 获取数据 */
