...
class PluginController extends AdminController {
	...
	public function create() {
		if (! is_writable ( ONETHINK_PLUGIN_PATH ))
			$this->error( '140173:您没有创建目录写入权限，无法使用此功能' );
...
str;
	...
	public function install() {
		...
		if (! $info || ! $addons->checkInfo ()) // 检测信息的正确性
			$this->error( '140181:插件信息缺失' );
		session ( 'addons_install_error', null );
		$install_flag = $addons->install ();
		if (! $install_flag) {
			$this->error( '140182:执行插件预安装操作失败' . session ( 'addons_install_error' ) );
		}
		$addonsModel = D ( 'Plugin' );
		$data = $addonsModel->create ( $info );
		if (! $data)
			$this->error( '140183:' . $addonsModel->getError () );
		...
		if ($addonsModel->add ( $data )) {
			...
			if ($hooks_update) {
				...
				$this->error( '140184:更新钩子处插件失败,请卸载后尝试重新安装' );
			}
		} else {
			$this->error( '140185:写入插件数据失败' );
		}
	}
	...
	public function uninstall() {
		...
		if (! $db_addons || ! class_exists ( $class ))
			$this->error( '140186:插件不存在' );
		session ( 'addons_uninstall_error', null );
		$addons = new $class ();
		$uninstall_flag = $addons->uninstall ();
		if (! $uninstall_flag)
			$this->error( '140187:执行插件预卸载操作失败' . session ( 'addons_uninstall_error' ) );
		$hooks_update = D ( 'Hooks' )->removeHooks ( $db_addons ['name'] );
		if ($hooks_update === false) {
			$this->error( '140188:卸载插件所挂载的钩子数据失败' );
		}
		S ( 'hooks', null );
		$delete = $addonsModel->where ( "name='{$db_addons['name']}'" )->delete ();
		if ($delete === false) {
			$this->error( '140189:卸载插件失败' );
		} else {
			$this->success ( '卸载成功' );
		}
	}
		...
		if ($data) {
			if ($data ['id']) {
				...
				if ($flag) {
					...
					$this->error( '140192:新增失败' );
				}
			}
		} else {
			$this->error( '140193:' . $hookModel->getError () );
		}
	}
	public function execute($_addons = null, $_controller = null, $_action = null) {
		...
		if (! empty ( $_addons ) && ! empty ( $_controller ) && ! empty ( $_action )) {
			...
			$this->error( '140194:没有指定插件名称，控制器或操作！' );
		}
	}
	public function edit() {
		...
		$param = $addon->admin_list;
		if (! $param)
			$this->error( '140196:插件列表信息不正确' );
		extract ( $param );
		$this->assign ( 'title', $addon->info ['title'] );
		if (isset ( $model )) {
			$addonModel = D ( "Plugins://{$name}/{$model}" );
			if (! $addonModel)
				$this->error( '140197:模型无法实列化' );
			$model = $addonModel->model;
			$this->assign ( 'model', $model );
		}
		if ($id) {
			$data = $addonModel->find ( $id );
			$data || $this->error( '140198:数据不存在！' );
		...
		if (IS_POST) {
			...
			if (! $addonModel->create ())
				$this->error( '140199:' . $addonModel->getError () );
			...
			if ($id) {
				...
				if ($flag !== false)
					...
					$this->error( '140200:' . $addonModel->getError () );
			} else {
				$flag = $addonModel->add ();
				if ($flag)
					$this->success ( "添加{$model['title']}成功！", Cookie ( '__forward__' ) );
			}
			$this->error( '140201:' . $addonModel->getError () );
		} else {
	...
	public function del($id = '', $name) {
		...
		$param = $addon->admin_list;
		if (! $param)
			$this->error( '140204:插件列表信息不正确' );
		extract ( $param );
		if (isset ( $model )) {
			$addonModel = D ( "Plugins://{$name}/{$model}" );
			if (! $addonModel)
				$this->error( '140205:模型无法实列化' );
		}
