...
class AddonsController extends AdminController {
	...
	public function create() {
		if (! is_writable ( ONETHINK_ADDON_PATH ))
			$this->error( '140006:您没有创建目录写入权限，无法使用此功能' );
...
str;
	...
	public function build() {
		...
		if ($data ['has_outurl']) {
			...
			$files [] = "{$addon_dir}View/";
			$files [] = "{$addon_dir}View/{$data['info']['name']}/";
		}
		$custom_config = trim ( $data ['custom_config'] );
		if ($custom_config)
...
class {$data['info']['name']}Controller extends ManageBaseController{
...
str;
	...
	public function adminList($name) {
		...
		if (! class_exists ( $class ))
			$this->error( '140009:插件不存在' );
		$addon = new $class ();
		$this->assign ( 'addon', $addon );
		$param = $addon->admin_list;
		if (! $param)
			$this->error( '140010:插件列表信息不正确' );
	...
	public function install() {
		...
		if (! class_exists ( $class ))
			$this->error( '140013:插件不存在' );
		$addons = new $class ();
		$info = $addons->info;
		if (! $info || ! $addons->checkInfo ()) // 检测信息的正确性
			$this->error( '140014:插件信息缺失' );
		
		$info ['type'] = 0;
		session ( 'addons_install_error', null );
		$install_flag = $addons->install ();
		if (! $install_flag) {
			$this->error( '140015:执行插件预安装操作失败' . session ( 'addons_install_error' ) );
		}
		$addonsModel = D ( 'Addons' );
		$data = $addonsModel->create ( $info );
		if (! $data)
			$this->error( '140016:' . $addonsModel->getError () );
		...
		if ($addonsModel->add ( $data )) {
			...
			if ($hooks_update) {
				...
				$this->error( '140017:更新钩子处插件失败,请卸载后尝试重新安装' );
			}
		} else {
			$this->error( '140018:写入插件数据失败' );
		}
	}
	...
	public function uninstall() {
		...
		if (! $db_addons || ! class_exists ( $class ))
			$this->error( '140019:插件不存在' );
		session ( 'addons_uninstall_error', null );
		$addons = new $class ();
		$uninstall_flag = $addons->uninstall ();
		if (! $uninstall_flag)
			$this->error( '140020:执行插件预卸载操作失败' . session ( 'addons_uninstall_error' ) );
		$hooks_update = D ( 'Hooks' )->removeHooks ( $db_addons ['name'] );
		if ($hooks_update === false) {
			$this->error( '140021:卸载插件所挂载的钩子数据失败' );
		}
		S ( 'hooks', null );
		$delete = $addonsModel->where ( "name='{$db_addons['name']}'" )->delete ();
		if ($delete === false) {
			$this->error( '140022:卸载插件失败' );
		} else {
			$this->success ( '卸载成功' );
		}
	}
		...
		if ($data) {
			if ($data ['id']) {
				...
				if ($flag) {
					...
					$this->error( '140025:新增失败' );
				}
			}
		} else {
			$this->error( '140026:' . $hookModel->getError () );
		}
	}
	...
	public function edit() {
		...
		$class = get_addon_class ( $name );
		if (! class_exists ( $class ))
			$this->error( '140028:插件不存在' );
		$addon = new $class ();
		$this->assign ( 'addon', $addon );
		$param = $addon->admin_list;
		if (! $param)
			$this->error( '140029:插件列表信息不正确' );
		extract ( $param );
		$this->assign ( 'title', $addon->info ['title'] );
		if (isset ( $model )) {
			$addonModel = D ( "Addons://{$name}/{$model}" );
			if (! $addonModel)
				$this->error( '140030:模型无法实列化' );
			$model = $addonModel->model;
			$this->assign ( 'model', $model );
		}
		if ($id) {
			$data = $addonModel->find ( $id );
			$data || $this->error( '140031:数据不存在！' );
		...
		if (IS_POST) {
			...
			if (! $addonModel->create ())
				$this->error( '140032:' . $addonModel->getError () );
			...
			if ($id) {
				...
				if ($flag !== false)
					...
					$this->error( '140033:' . $addonModel->getError () );
			} else {
				$flag = $addonModel->add ();
				if ($flag)
					$this->success ( "添加{$model['title']}成功！", Cookie ( '__forward__' ) );
			}
			$this->error( '140034:' . $addonModel->getError () );
		} else {
	...
	public function del($id = '', $name) {
		...
		if (empty ( $ids )) {
			$this->error( '140035:请选择要操作的数据!' );
		}
		
		$class = get_addon_class ( $name );
		if (! class_exists ( $class ))
			$this->error( '140036:插件不存在' );
		$addon = new $class ();
		$param = $addon->admin_list;
		if (! $param)
			$this->error( '140037:插件列表信息不正确' );
		extract ( $param );
		if (isset ( $model )) {
			$addonModel = D ( "Addons://{$name}/{$model}" );
			if (! $addonModel)
				$this->error( '140038:模型无法实列化' );
		}
