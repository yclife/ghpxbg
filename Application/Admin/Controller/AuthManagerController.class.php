...
class AuthManagerController extends AdminController {
	...
	function set_switch() {
		...
		$map ['name'] = $arr [1];
		$save [$arr [0]] = $val;
		$res = M ( 'apps_auth' )->where ( $map )->save ( $save );
	...
	public function writeGroup() {
		...
		if ($data) {
			...
			if ($r === false) {
				$this->error( '140072:操作失败' . $AuthGroup->getError () );
			} else {
				$this->success ( '操作成功!', U ( 'index' ) );
			}
		} else {
			$this->error( '140073:操作失败' . $AuthGroup->getError () );
		}
	}
	...
	public function addToGroup() {
		...
		$gid = I ( 'group_id' );
		if (empty ( $uid )) {
			$this->error( '140077:参数有误' );
		}
		$AuthGroup = D ( 'AuthGroup' );
		if (is_numeric ( $uid )) {
			if (is_administrator ( $uid )) {
				$this->error( '140078:该用户为超级管理员' );
			}
			if (! M ( 'User' )->where ( array (
					'uid' => $uid 
			) )->find ()) {
				$this->error( '140079:用户不存在' );
			}
		}
		
		if ($gid && ! $AuthGroup->checkGroupId ( $gid )) {
			$this->error( '140080:' . $AuthGroup->error );
		}
		if ($AuthGroup->addToGroup ( $uid, $gid )) {
			$this->success ( '操作成功' );
		} else {
			$this->error( '140081:' . $AuthGroup->getError () );
		}
	}
	...
	public function removeFromGroup() {
		...
		$gid = I ( 'group_id' );
		if ($uid == UID) {
			$this->error( '140082:不允许解除自身授权' );
		}
		if (empty ( $uid ) || empty ( $gid )) {
			$this->error( '140083:参数有误' );
		}
		$AuthGroup = D ( 'AuthGroup' );
		if (! $AuthGroup->find ( $gid )) {
			$this->error( '140084:用户组不存在' );
		}
		if ($AuthGroup->removeFromGroup ( $uid, $gid )) {
			$this->success ( '操作成功' );
		} else {
			$this->error( '140085:操作失败' );
		}
	}
	...
	public function addToCategory() {
		...
		$gid = I ( 'group_id' );
		if (empty ( $gid )) {
			$this->error( '140086:参数有误' );
		}
		$AuthGroup = D ( 'AuthGroup' );
		if (! $AuthGroup->find ( $gid )) {
			$this->error( '140087:用户组不存在' );
		}
		if ($cid && ! $AuthGroup->checkCategoryId ( $cid )) {
			$this->error( '140088:' . $AuthGroup->error );
		}
		if ($AuthGroup->addToCategory ( $gid, $cid )) {
			$this->success ( '操作成功' );
		} else {
			$this->error( '140089:操作失败' );
		}
	}
	...
	public function addToModel() {
		...
		$gid = I ( 'get.group_id' );
		if (empty ( $gid )) {
			$this->error( '140090:参数有误' );
		}
		$AuthGroup = D ( 'AuthGroup' );
		if (! $AuthGroup->find ( $gid )) {
			$this->error( '140091:用户组不存在' );
		}
		if ($mid && ! $AuthGroup->checkModelId ( $mid )) {
			$this->error( '140092:' . $AuthGroup->error );
		}
		if ($AuthGroup->addToModel ( $gid, $mid )) {
			$this->success ( '操作成功' );
		} else {
			$this->error( '140093:操作失败' );
		}
	}
}
