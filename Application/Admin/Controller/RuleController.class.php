...
class RuleController extends AdminController {
	...
	public function writeRule() {
		...
		if ($data) {
			...
			if ($r === false) {
				$this->error( '140238:操作失败' . $dao->getError () );
			} else {
				$url = $data ['group'] == 'wechat' ? U ( 'wechat' ) : U ( 'index' );
				$this->success ( '操作成功!', $url );
			}
		} else {
			$this->error( '140239:操作失败' . $dao->getError () );
		}
	}
	...
	public function addToGroup() {
		...
		$gid = I ( 'group_id' );
		if (empty ( $uid )) {
			$this->error( '140243:参数有误' );
		}
		$AuthGroup = D ( 'AuthGroup' );
		if (is_numeric ( $uid )) {
			if (is_administrator ( $uid )) {
				$this->error( '140244:该用户为超级管理员' );
			}
			if (! M ( 'User' )->where ( array (
					'uid' => $uid 
			) )->find ()) {
				$this->error( '140245:用户不存在' );
			}
		}
		
		if ($gid && ! $AuthGroup->checkGroupId ( $gid )) {
			$this->error( '140246:' . $AuthGroup->error );
		}
		if ($AuthGroup->addToGroup ( $uid, $gid )) {
			$this->success ( '操作成功' );
		} else {
			$this->error( '140247:' . $AuthGroup->getError () );
		}
	}
	...
	public function removeFromGroup() {
		...
		$gid = I ( 'group_id' );
		if ($uid == UID) {
			$this->error( '140248:不允许解除自身授权' );
		}
		if (empty ( $uid ) || empty ( $gid )) {
			$this->error( '140249:参数有误' );
		}
		$AuthGroup = D ( 'AuthGroup' );
		if (! $AuthGroup->find ( $gid )) {
			$this->error( '140250:用户组不存在' );
		}
		if ($AuthGroup->removeFromGroup ( $uid, $gid )) {
			$this->success ( '操作成功' );
		} else {
			$this->error( '140251:操作失败' );
		}
	}
	...
	public function addToCategory() {
		...
		$gid = I ( 'group_id' );
		if (empty ( $gid )) {
			$this->error( '140252:参数有误' );
		}
		$AuthGroup = D ( 'AuthGroup' );
		if (! $AuthGroup->find ( $gid )) {
			$this->error( '140253:用户组不存在' );
		}
		if ($cid && ! $AuthGroup->checkCategoryId ( $cid )) {
			$this->error( '140254:' . $AuthGroup->error );
		}
		if ($AuthGroup->addToCategory ( $gid, $cid )) {
			$this->success ( '操作成功' );
		} else {
			$this->error( '140255:操作失败' );
		}
	}
	...
	public function addToModel() {
		...
		$gid = I ( 'get.group_id' );
		if (empty ( $gid )) {
			$this->error( '140256:参数有误' );
		}
		$AuthGroup = D ( 'AuthGroup' );
		if (! $AuthGroup->find ( $gid )) {
			$this->error( '140257:用户组不存在' );
		}
		if ($mid && ! $AuthGroup->checkModelId ( $mid )) {
			$this->error( '140258:' . $AuthGroup->error );
		}
		if ($AuthGroup->addToModel ( $gid, $mid )) {
			$this->success ( '操作成功' );
		} else {
			$this->error( '140259:操作失败' );
		}
	}
}
