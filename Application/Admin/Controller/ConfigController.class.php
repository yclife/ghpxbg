...
class ConfigController extends AdminController {
	...
	public function index() {
		...
		$list = $this->lists_admin( 'config', $map, 'id' );
		// 记录当前列表页的cookie
		...
		if (IS_POST) {
			...
			if ($data) {
				if ($Config->add ()) {
					...
					$this->error ( '140105:新增失败' );
				}
			} else {
				$this->error ( '140106:' . $Config->getError () );
			}
		} else {
		...
		if (IS_POST) {
			...
			if ($data) {
				if ($Config->save ()) {
					...
					$this->error ( '140107:更新失败' );
				}
			} else {
				$this->error ( '140108:' . $Config->getError () );
			}
		} else {
			$info = array ();
			/* 获取数据 */
	...
	function test_mail() {
		if (IS_POST) {
			...
			if ($result === false) {
				$this->error ( '140112:' . $mail->message );
			} else {
				$this->success ( '发送成功' );
			}
			exit ();
	...
	public function sort() {
		if (IS_GET) {
			...
			if ($res !== false) {
				...
				$this->error ( '140113:排序失败！' );
			}
		} else {
			$this->error ( '140114:非法请求！' );
		}
	}
}