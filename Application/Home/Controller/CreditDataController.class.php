...
class CreditDataController extends HomeController {
	...
	public function lists() {
		...
		if (! empty ( $_GET ['uid'] )) {
			...
			if ($uids) {
				...
				// dump($map1);
				$uids1 = M ( 'apps_follow' )->where ( $map1 )->getField ( 'uid', true );
				$where ['uid'] = array (
						'in',
						$uids1 
				);
	...
	public function add() {
		...
		if (IS_POST) {
			...
			if ($Model->create () && $id = $Model->add ()) {
				...
				$this->error ( '110071:' . $Model->getError () );
			}
		} else {
	...
	public function edit() {
		...
		if (IS_POST) {
			...
			if ($res !== false ) {
				...
				// dump($Model->getLastSql());
				$this->error ( '110073:' . $Model->getError () );
			}
		} else {
			$fields = get_model_attribute ( $model ['id'] );
	...
	function import() {
		...
		if (IS_POST) {
			...
			$this->success ( $msg, U ( 'lists', array (
					'uid' => $uidStr 
			) ) );
		} else {
			$fields = get_model_attribute ( $model ['id'] );
