...
class MessageController extends HomeController {
	...
	public function add() {
		...
		if (IS_POST) {
			if (! C ( 'SEND_GROUP_MSG' )) {
				$this->error ( '110140:抱歉，您的公众号没有群发消息的权限' );
			}
			...
			if ($Model->create () && $id = $Model->add ()) {
				...
				$this->error ( '110141:' . $Model->getError () );
			}
		} else {
...
2、对于认证服务号虽然使用高级群发接口的每日调用限制为100次，但是用户每月只能接收4条，无论在公众平台网站上，还是使用接口群发，用户每月只能接收4条群发消息，多于4条的群发将对该用户发送失败；' );
	...
	function preview() {
		if (! C ( 'SEND_GROUP_MSG' )) {
			$this->error ( '110142:抱歉，您的公众号没有群发消息的权限' );
		}
		$openids = wp_explode ( I ( 'preview_openids' ) );
		if (empty ( $openids )) {
			$this->error ( '110143:预览OpenID不能为空' );
		}
		...
		if ($count == 0) {
			...
			$this->error ( '110144:有 ' . $count . '条信息发送失败' );
		}
	}
	// 按用户组发送
	function _send_by_group($group_id) {
		...
		if ($res ['errcode'] != 0) {
			$this->error ( '110145:' . error_msg ( $res ) );
		} else {
			return $res ['msg_id'];
		}
	}
	...
	function _send_by_openid($openids) {
		...
		if (empty ( $openids )) {
			$this->error ( '110146:要发送的OpenID值不能为空' );
		}
		if (count ( $openids ) < 2) {
			$this->error ( '110147:OpenID至少需要2个或者2个以上' );
		}
		...
		$res = post_data ( $url, $param );
		if ($res ['errcode'] != 0) {
			$this->error ( '110148:' . error_msg ( $res ) );
		} else {
			return $res ['msg_id'];
		}
	}
	...
	function custom_sendall() {
		...
		if (IS_POST) {
			...
			if ($sendType == 1 && $sendOpenid == '') {
				$this->error ( '110154:指定的Openid值不能为空' );
			}
			...
			if ($count == 0) {
				...
				$this->error ( '110161:有 ' . $count . ' 条消息 发送失败！' );
			}
		} else {
			$map ['token'] = get_token ();
			$map ['manager_id'] = $this->mid;
	...
	function _get_user_openid($sendType = 0, $groupId = 0, $openidStr = '') {
		$map ['has_subscribe'] = 1;
		$map ['token'] = get_token ();
		$allUser = M ( 'apps_follow' )->where ( $map )->getFields ( 'uid,openid' );
		...
		if ($sendType == 0 && $groupId == 0) {
			...
			foreach ( $openids as $op ) {
				...
				if ($uid) {
					...
					$this->error ( '110162:Openid为: ' . $op . ' 的用户不存在' );
				}
			}
			return $data;
		}
			...
			foreach ( $list ['list_data'] as &$v ) {
				...
				if ($v ['msgType'] == 'text') {
					...
					foreach ( $appMsgData as $vo ) {
						...
						if (empty ( $vo ['url'] )) {
							$art_url = U ( 'Home/Wap/news_detail', array (
									'id' => $vo ['id'] 
							) );
							// 文章内容
